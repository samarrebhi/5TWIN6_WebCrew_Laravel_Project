...
<div class="container mt-5">
    ...
    <!-- Search Field -->
    <div class="mb-3">
        <input type="text" id="search-center" class="form-control" placeholder="Search centers by name...">
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#search-center').on('keyup', function() {
            let query = $(this).val();

            // AJAX request for search
            $.ajax({
                url: "{{ route('center.search') }}",
                method: 'GET',
                data: { query: query },
                success: function(response) {
                    $('#centers-list').html(response);  // Update the table with search results
                },
                error: function(xhr) {
